...
export async function solveSudoku(
  ...
  // CONTROLLED DELAY
  async function controlledDelay(ms: number) {
  ...
  // VALIDATION
  function isValid(board: Board, row: number, col: number, num: number) {
  ...
  function getBestCell() {
    ...
    return { row: bestRow, col: bestCol, options: bestOptions };
  }
  ...
  async function solve(): Promise<boolean> {
    ...
    // ONLY SHOW STATUS IF NOT PAUSED
    if (!pauseRef.current) {
      setStatus(`AI analyzing Row ${row + 1}, Column ${col + 1}`);
    }

    await controlledDelay(850);
    ...
    for (const num of options) {
      ...
      setStatus(`Placed ${num} at Row ${row + 1}, Column ${col + 1}`);

      await controlledDelay(750);
